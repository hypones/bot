...
exports.run = async (client, message, args) => {
  if (!message.member.hasPermission("MANAGE_NICKNAMES"))
    return message.channel.send(
      new Discord.MessageEmbed()
        .setColor("RANDOM")
        .setDescription(
          `Bu Komutu Kullanabilmek İçin **Kullanıcı Adlarını Yönet** Yetkisine Sahip Olmalısın.`
        )
    );
  ...
  message.channel.send(
    new Discord.MessageEmbed()
      ...
      .addField(
        "İşlem Başarılı",
        `${salvokullanıcı} İsimli Kullanıcının Yeni İsmi **${salvoisim}\** Olarak Ayarlandı!`
      )
  );
};
